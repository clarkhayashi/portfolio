...
export function physicalAction(g,r,player,a){
 const p=r.physical;if(!p||!['physicalSetup','physical','physicalConfirm'].includes(r.phase))throw Error('This activity has ended.');
 ...
 if(a.type==='physicalCategory'){
  ...
  if(!host||r.phase==='physicalConfirm')throw Error('Only the host can start the beat.');
  ...
  if(!host||r.phase!=='physical'||p.practice)throw Error('Only the host can finish a live round.');physicalStop(r);p.confirmed=[];g.phase(r,'physicalConfirm');
  ...
  p.confirmed=[];physicalStop(r);g.phase(r,'physical');
