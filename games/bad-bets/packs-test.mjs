...
test('lobby panels escape text, label adult packs, and tile art falls back to the icon',async()=>{const ui=await import('./public/ui.js');
 ...
 const html=ui.packsPanel(s,true);assert.ok(!html.includes('<script>')&&!html.includes('<img src=x')&&!html.includes('<i>Al'));assert.ok(html.includes('Spicy (adults)'));assert.equal(typeof ui.customPanel,'undefined','the old lobby Add your own panel is gone');assert.ok(ui.art('finale').includes('/art/finale.svg')&&ui.art('finale').includes('logo-mark.svg'));
