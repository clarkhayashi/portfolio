...
test('missing quip refunds instead of awarding unopposed win',()=>{const {g,r,p}=setup();r.mode='tournament';r.active=p.slice(0,2).map(p=>p.id);g.action(r,p[0],{type:'submit',value:'Hello'});g.advance(r);p.slice(2).forEach(q=>g.action(r,q,{type:'vote',player:p[0].id}));assert.equal(r.result.tie,true);});
...
test('no immediate repeat when another compatible game remains',()=>{const {g,r}=setup(4,'brain','tournament');r.enabledGames=['brain','number'];r.previous='brain';r.candidates=['brain','number'];g.choose(r);assert.equal(r.game,'number');});
...
test('creative Party draws when fewer than two independent judges remain',()=>{const {g,r,p}=setup(4);r.mode='tournament';r.active=p.slice(0,2).map(q=>q.id);r.submissions=Object.fromEntries(r.active.map(id=>[id,'Answer']));g.openVote(r);g.action(r,p[2],{type:'leave'});g.action(r,p[3],{type:'vote',player:p[0].id});assert.equal(r.result.tie,true);});
