...
test('auction raises can jump by $2 or $5 but never past the budget you must keep',()=>{const {g,r,p}=setup(2,'auction');const a=r.auction,q=p.find(x=>x.id===a.turn),o=p.find(x=>x!==q);const start=a.bid;auction(g,r,q,{type:'auctionRaise',by:5});assert.equal(a.bid,start+5);assert.equal(a.leader,q.id);
 assert.throws(()=>auction(g,r,o,{type:'auctionRaise',by:0}),/at least/);assert.throws(()=>auction(g,r,o,{type:'auctionRaise',by:2.5}),/at least/);assert.throws(()=>auction(g,r,o,{type:'auctionRaise',by:999}),/Keep \$1/);auction(g,r,o,{type:'auctionRaise',by:2});assert.equal(a.bid,start+7);assert.equal(a.leader,o.id);});
