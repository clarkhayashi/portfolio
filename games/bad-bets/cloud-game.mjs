...
export async function cloudRequest(store,action,{state=false}={}){
 ...
 if(action.type==='create'&&!state){
  for(let attempt=0;attempt<8;attempt++){
   const game=new Game();const result=game.create(action.name,action.mode,typeof action.quick==='string'?action.quick:null,action.quickGame);const room=game.rooms.get(result.code);
