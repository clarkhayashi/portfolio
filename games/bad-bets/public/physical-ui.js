...
export function physicalScreen(s){const p=s.physical,n=id=>esc(s.players.find(p=>p.id===id)?.name||'Player'),host=s.you===s.host,keeper=s.you===p.keeper||host,shadow=s.game==='shadow';let h=`<div class="eyebrow">${shadow?'IN-PERSON FACE-OFF':'AROUND THE ROOM'}</div><h2>${shadow?'Shadowbox':'Keep It Going'}</h2>`;
 if(s.phase==='physicalSetup'){
  ...
  if(s.phase==='physicalConfirm')h+=`<h3>Does this score look right?</h3><p>${p.confirmed.length}/${s.active.length} players confirmed. Accepts automatically in 10 seconds. Tap Fix the score to pause.</p>${s.active.includes(s.you)?`<div class="row">${button(p.confirmed.includes(s.you)?'Confirmed':'Confirm score','physicalConfirm',p.confirmed.includes(s.you))}${button('Fix the score','physicalDispute',false,'secondary')}</div>`:'<p>Waiting for the players to confirm.</p>'}`;
  else h+=`<div class="row physical-actions">${!p.paused?button('Pause','physicalPause',false,'secondary'):host?button(p.practice?'Start real round':'Resume beat','physicalStart'):''}${shadow&&keeper&&!p.practice?button('Dodged · switch attacker','physicalDodge'):''}${keeper&&p.canUndo?button('Undo last change','physicalUndo',false,'secondary'):''}${host&&!p.practice?button('Finish & review scores','physicalFinish',false,'secondary'):''}</div><p>${p.practice?'Listen for two low beeps and one high beep.':shadow?'Record a hit with +. A dodge changes the attacker.':'A repeat, missed turn, or invalid answer counts as one mistake. Pause for disagreements.'}</p>`;
 }
 return h;
}
setInterval(()=>{
