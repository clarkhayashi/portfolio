...
export function headshot(name){const p=players&&Object.hasOwn(players,name)?players[name]:null;
 if(p&&typeof p.file==='string'&&/^\/players\/[a-z0-9-]+\.jpg$/.test(p.file))return `<span class="card-art headshot" data-fallback="${esc(jerseyCard(name))}"><img src="${esc(p.file)}" alt="" width="80" height="80" loading="lazy" decoding="async" onerror="this.parentNode.outerHTML=this.parentNode.dataset.fallback"></span>`;
