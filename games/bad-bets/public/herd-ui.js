...
export function herdScreen(s,{esc,name,btn,heading,host}){
 ...
 if(s.phase==='reveal')return heading('HERD ROUND','Same Brain? · Herd round')+`<p class="center">${HERD_RULE}</p><div class="note">${h.youAsk?'You’re the writer this time. Get a this-or-that ready.':`${writer} writes the question. You pick a side.`}</div><p>${s.mode==='minigames'?'No chips in this game.':h.youAsk?`You sit out the pot. Only a too-easy question costs you (up to ${h.penalty} chips).`:`5 chips to enter. The bigger side splits the pot. A tie gives everyone their 5 back. The Black Sheep pays up to ${h.penalty} more.`}</p>`;
 ...
 if(s.phase==='herdVote'){
  const top=heading('PICK A SIDE',esc(h.question))+(h.stock?'<p class="micro">Built-in question</p>':`<p class="micro">Written by ${writer}</p>`);
 ...
 if(!o||o.cancelled)return heading('RESULT','Round over')+`<p>${esc(r.detail)}</p>${chips}${next}`;
 ...
 return heading('RESULT',title)+`<p class="vote-prompt">${esc(h.question)}</p>${call}<div class="herd-cols">${col('a',o.A)}${col('b',o.B)}</div>${o.missed.length?`<p class="micro">No pick: ${o.missed.map(name).join(', ')}</p>`:''}<p>${esc(r.detail)}</p>${s.mode==='minigames'?`<p>${o.tie?'Tied round':r.winners.length?'Winner'+(r.winners.length>1?'s':'')+': '+r.winners.map(name).join(', '):'No winner this time'}</p>`:chips}${next}`;
}
